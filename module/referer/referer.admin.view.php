...
*/
	...
	function prepareUAStatData($data) {
		$Types = array('desktop' => 0, 'mobile' => 0, 'bot' => 0, 'notbot' => 0);
		$Bots = array();
		$Browsers = array();
		$OSes = array();
		$IEs = array();
		...
		foreach($data as $no => $val) {
			...
			if ( preg_match('/(googlebot)/i',$uagent) || stripos($uagent, 'Google favicon') ) $bot = "Google";
				...
				|| strstr($val->uagent, 'MSIE or Firefox mutant; not on Windows server;')) ) $bot = "Others";
			...
			if ($bot != "") {
				...
				else if ( stripos($uagent, "Windows Phone") )
				{
					...
					if ( stripos($uagent, "MSIE 5") ) $ie = "MSIE 5";
					...
					else if ( stripos($uagent, "Trident/7.0") ) $ie = "MSIE 11";
				...
				if ($browser == "") {
					if ( stripos($uagent, "NAVER") ) $browser = "Naver App";
					else if ( stripos($uagent, "DaumApps") ) $browser = "Daum App";
					...
					else if ( stripos($uagent, "Avant") ) $browser = "Avant Browser";
